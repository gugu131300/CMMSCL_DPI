...
protein_id = np.loadtxt("E:/OneDrive/桌面/new_paper/dataset/GPCR/check390/nonD92M_unid.txt", dtype=str)
uniport_id2pdb_id = pd.read_csv("E:/OneDrive/桌面/new_paper/dataset/GPCR/check390/nonD92M_pidmapping.tsv", sep="\t", header=0)
...
def download_af_pdb(p_id_without_pdb):
    for p_name in tqdm(p_id_without_pdb):
        ...
        save_path = "E:/OneDrive/桌面/new_paper/dataset/GPCR/check390/pdb_file/AF-" + p_name + "-F1-model_v4.pdb"
...
def download_pdb(uniport_id2pdb_id):
    for i in tqdm(uniport_id2pdb_id.To):
        ...
        save_path = "E:/OneDrive/桌面/new_paper/dataset/GPCR/check390/pdb_file/" + i + ".pdb"
        if not os.path.exists(save_path):
...
# 2. 检查下载的文件是否正确，重新下载错误的pdb数据
# 对于不存在pdb格式的pdb id，删除该id以及对应的假pdb文件
def redownload_pdb(uniport_id2pdb_id):
    ...
    for line_i in tqdm(range(len(uniport_id2pdb_id))):
        ...
        download_path= "https://files.rcsb.org/download/" + pdb_id + ".pdb"
        save_path = "E:/OneDrive/桌面/new_paper/dataset/GPCR/check390/pdb_file/" + pdb_id + ".pdb"
        if not os.path.exists(save_path):
            pdb_file = requests.get(download_path)
            pdb_data = pdb_file.content
            open(save_path, "wb").write(pdb_data)
...
def get_best_pdb(downloaded_uniport_id2pdb_id,p_id_without_pdb,p_id_with_pdb):
    ...
    for p_name in tqdm(p_id_with_pdb):
        ...
        for pdb_name in pdb_names:
            pdb_path = "E:/OneDrive/桌面/new_paper/dataset/GPCR/check390/pdb_file/" + pdb_name + ".pdb"
        ...
        best_pdb_path = "E:/OneDrive/桌面/new_paper/dataset/GPCR/check390/pdb_file/" + best_pdb_name[0] + ".pdb"
        best_pdb_save_path = "E:/OneDrive/桌面/new_paper/dataset/GPCR/check390/best_pdb_file/" + p_name + ".pdb"
    ...
    for p_name in tqdm(p_id_without_pdb):
        pdb_path = "E:/OneDrive/桌面/new_paper/dataset/GPCR/check390/pdb_file/AF-" + p_name + "-F1-model_v4.pdb"
        best_pdb_save_path = "E:/OneDrive/桌面/new_paper/dataset/GPCR/check390/best_pdb_file/" + p_name + ".pdb"
...
# max_len = len(uniport_id2pdb_id)
# # 创建 Thread 实例下载pdb数据
# t = []
# for i in range(10):
#     batch_len = max_len / 10
#     batch_len = int(batch_len)
#     if i < 9:
#         uniport_id2pdb_id_dataframes = uniport_id2pdb_id.iloc[batch_len*i:batch_len*(i+1),:]
#         thread_temp = Thread(target=download_pdb, args=(uniport_id2pdb_id_dataframes,))
#         thread_temp.start()
#         t.append(thread_temp)
#     else:
#         uniport_id2pdb_id_dataframes = uniport_id2pdb_id.iloc[batch_len * i:, :]
#         thread_temp = Thread(target=download_pdb, args=(uniport_id2pdb_id_dataframes,))
#         thread_temp.start()
#         t.append(thread_temp)
# # 启动线程运行
