...
from sklearn.metrics import average_precision_score, precision_score, recall_score, roc_auc_score, accuracy_score
...
def test(model, test_loader, args, compound_graph, protein_graph, protein_embedding,
    ...
    if epoch % 1 == 0:
        # np.savetxt("D84/D84_result_eps/total_combined_fea_test_{}.txt".format(epoch + 1), total_fea_test.detach())
        # np.savetxt("D84/D84_result_eps/total_label_test_{}.txt".format(epoch + 1), total_labels)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_path', default='/home/zqguxingyue/DTI_D92M/PPI/D92M_classification.csv', help='dataset path')
    parser.add_argument('--PPI_path', default='/home/zqguxingyue/DTI_D92M/PPI/', help='PPI')
    parser.add_argument('--dataset', default='D92M', help='dataset')
    parser.add_argument('--foldpath', default='/home/zqguxingyue/DTI_D92M/PPI/fold_id/', help='foldpath')
    ...
    if not dataset_exists:
        ...
        for train_index, test_index in kf.split(data_shuffled):
            ...
            DPI_label = np.load(args.foldpath + str(fold_index + 1) + 'label_train.npy')
        ...
        test_loader = DataLoader(testDTI, batch_size=args.batch, shuffle=False, collate_fn=testDTI.collate,
                                 drop_last=True)
        # todo 加载去重后的protein seq和graph
        # todo 加载去重后的compound smiles和graph
        # todo 加载去重后的protein 氨基酸特征
        # todo 加载去重后的compound 原子特征
