...
def Kfold(train_index, test_index, args, data_shuffled, fold_index):
    ...
    for no, id in enumerate(train_index):
        protein_graph_train, _ = load_graphs(
            args.PPI_path + 'protein_graph/' + data_shuffled.iloc[id]['PROTEIN_ID'] + '.bin')
    ...
    for no, id in enumerate(train_index):
        compound_graph_train, _ = load_graphs(
            args.PPI_path + 'compound_graph_id/' + str(data_shuffled.iloc[id]['COMPOUND_ID']) + '.bin')
    ...
    for no, id in enumerate(train_index):
        label_train.append(data_shuffled.iloc[id]['LABEL'])
    np.save(args.foldpath + str(fold_index) + 'label_train.npy', label_train)
    for no, id in enumerate(test_index):
        label_test.append(data_shuffled.iloc[id]['LABEL'])
...
#     dist_matrix = np.sum(x1**2) + np.sum(x2**2) - 2 * np.dot(x1, x2.T)
