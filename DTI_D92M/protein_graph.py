...
def Protein_graph_construction(id, dir_output):
    ...
    for no, data in enumerate(id):
        ...
        pdb_pdb = '/home/zqguxingyue/DTI/' + dataset + '/D84_best_pdb_file/' + data + '.pdb'
...
def Protein_graph_process(dataset, dir_output, id_train):
    proteins_graph_train, proteins_graph_test = [], []

    N = len(id_train)
    for no, id in enumerate(id_train):
        print('/'.join(map(str, [no + 1, N])))
        protein_graph_train, _ = load_graphs('/home/zqguxingyue/DTI/' + dataset + '/pocket_graph/' + str(id) + '.bin')
        proteins_graph_train.append(protein_graph_train[0])
    print(len(proteins_graph_train))
    dgl.save_graphs(dir_output + '/protein_graph.bin', proteins_graph_train)

def Protein_id_process(dir_output, id_train):
    proteins_id_train, proteins_id_test = [], []
    N = len(id_train)
    for no, id in enumerate(id_train):
        print('/'.join(map(str, [no + 1, N])))
        proteins_id_train.append(id)
    np.save(dir_output + '/train' + '/protein_id.npy', proteins_id_train)

if __name__ == '__main__':
    dataset = 'D84'
    #file_path = 'E:/OneDrive/桌面/new_paper/dataset/' + dataset + '.csv'
    file_path_protein = '/home/zqguxingyue/DTI/' + dataset + '/' + dataset + '.csv'
    ...
    protein_id_unique = raw_data_protein['PROTEIN_ID'].drop_duplicates().values

    N = len(protein_id_unique)

    Protein_graph_construction(id=protein_id_unique, dir_output=dir_output)

    train_data = pd.read_csv(file_path_protein + '/' + dataset + '_train.csv')

    protein_id_train = train_data['PROTEIN_ID'].values

    Protein_graph_process(dataset=dataset, id_train=protein_id_train, dir_output=dir_output)
    #Protein_embedding_process(dataset=dataset, id_train=protein_id_train, id_test=protein_id_test, dir_output=dir_output)
    Protein_id_process(dataset=dataset, id_train=protein_id_train, dir_output=dir_output)
