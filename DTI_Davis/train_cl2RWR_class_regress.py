...
def test(model, test_loader, args, compound_graph, protein_graph, protein_embedding,
    ...
    np.save("/home/zqguxingyue/DTI_Davis/Davis_log/total_pred_labels.npy", total_pred_labels)
    np.save("/home/zqguxingyue/DTI_Davis/Davis_log/total_pred_scores.npy", total_pred_scores)
    np.save("/home/zqguxingyue/DTI_Davis/Davis_log/compound_smiles_ids.npy", compound_smiles_ids)
    np.save("/home/zqguxingyue/DTI_Davis/Davis_log/protein_seq_ids.npy", protein_seq_ids)
    ...
    # 计算残差
    residuals = total_labels - total_pred_scores
    # 计算标准误差 (Standard Error)
    se = np.sqrt(np.sum(residuals ** 2) / len(residuals))
    # 使用1.96倍的标准误差来计算95%置信区间
    ci_test = 1.96 * se
    mse_test = mean_squared_error(total_labels, total_pred_scores)
    rmse_test = np.sqrt(mse_test)
    rm2_test = r2_score(total_labels, total_pred_scores)
    # tpr, fpr, _ = precision_recall_curve(total_labels, total_pred_scores)
    # AUPR = auc(fpr, tpr)

    if epoch % 1 == 0:
        np.savetxt("Davis_result_eps/total_combined_fea_test_{}.txt".format(epoch + 1), total_fea_test.detach())
        np.savetxt("Davis_result_eps/total_label_test_{}.txt".format(epoch + 1), total_labels)
    ...
    return mse_test, rmse_test, ci_test, rm2_test
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dropout', type=float, default=0.5, help='Dropout rate (1 - keep probability).')  # 0.1
    ...
    parser.add_argument('--dataset_path', default='/home/zqguxingyue/DTI_Davis/PPI/Davis_classification.csv',
                        help='dataset path')
    parser.add_argument('--PPI_path', default='/home/zqguxingyue/DTI_Davis/PPI/', help='PPI')
    parser.add_argument('--dataset', default='Davis', help='dataset')
    parser.add_argument('--foldpath', default='/home/zqguxingyue/DTI_Davis/PPI/fold_id/', help='foldpath')

    args = parser.parse_args()
    # 打印参数及其值
    print("gt_heads", args.gt_heads)
    print("gt_layers", args.gt_layers)
    ...
    best_epoch = -1
    best_mse = float('inf')  # 初始化为无穷大，表示最小化的指标
    best_rmse = float('inf')  # 同上
    best_ci = float('inf')  # 同上
    best_rm2 = -float('inf')  # 初始化为负无穷大，因为R²越大越好
    ...
    if not dataset_exists:
        ...
        for train_index, test_index in kf.split(data_shuffled):
            ...
            # print("if not dataset_exists device:{}".format(args.device))
            ...
            for epoch in range(args.epochs):
                ...
                mse_test, rmse_test, ci_test, rm2_test = test(model, test_loader, args,
                                                              compound_graph, protein_graph, protein_embedding,
                                                              DPI_label)
                scheduler.step(mse_test)
                end = timeit.default_timer()
                time = end - start
                print("epoch     time     mse_test     rmse_test    ci_test    rm2_test")
                ret = [epoch + 1, round(time, 2), round(mse_test, 5), round(rmse_test, 5), round(ci_test, 5),
                       round(rm2_test, 5)]
        ...
        for epoch in range(args.epochs):
            ...
            if mse_test < best_mse:
                ...
                best_ci = ci_test
                best_rm2 = rm2_test
                print('MSE improved at epoch ', best_epoch, ';\tbest_mse:', best_mse, ';\tbest_rmse:',
                      best_rmse, ';\tbest_ci:', best_ci, ';\tbest_rm2:', best_rm2, )