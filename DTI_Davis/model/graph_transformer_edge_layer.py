...
class MultiHeadAttentionLayer(nn.Module):
    def __init__(self, in_dim, out_dim, num_heads, use_bias):
        ...
        if use_bias:
            ...
            self.Q = nn.Linear(in_dim, int(out_dim * num_heads), bias=False)
            self.K = nn.Linear(in_dim, int(out_dim * num_heads), bias=False)
            self.V = nn.Linear(in_dim, int(out_dim * num_heads), bias=False)
            self.proj_e = nn.Linear(in_dim, int(out_dim * num_heads), bias=False)
...
class GraphTransformerLayer(nn.Module):
    """
        Param: 
    """
    def __init__(self, device, in_dim, out_dim, num_heads, dropout, layer_norm=True, batch_norm=False, residual=True, use_bias=False):
        super().__init__()
        ...
        self.O_h = nn.Linear(63, out_dim)
        self.O_e = nn.Linear(63, out_dim)
    ...
    def forward(self, g, h, e):
        ...
        h_attn_out, e_attn_out = self.attention(self.device, g, h, e)

        # print("h_attn_out", h_attn_out.size())
        # print("e_attn_out", e_attn_out.size())
        
        # h = h_attn_out.view(-1, self.out_channels)
        # e = e_attn_out.view(-1, self.out_channels)
        h = h_attn_out.view(-1, 63)
        e = e_attn_out.view(-1, 63)
